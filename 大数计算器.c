...
/*void chu_c_b_neicun(char *x,char *y,int lenx,const int leny,int float_x,int float_y,char *result,int result_long,int baoliu)
{
    if(lenx==0)
    {
        return;
    ...
    if(leny>=19)
    {
        char temp_y[leny+1],temp_x[leny+1];
        if(lenx>leny)
        {
            int jindu_x=leny+1;
            float_gong=result_long-baoliu-1+leny-lenx+float_x-float_y;


            if(fake_strncmp(x,y,leny)>=0)
            {
                float_gong--;
                if(float_gong>=result_long)
                {
                    ...
                    temp_x[0]=0;
                    memcpy(x,temp_x+1,leny);
                    result[float_gong]=part_chu_t(temp_x,y,leny,temp_y);
                
                
                
                    for(int i=0;i<leny;i++)
                    {
                        x[i]-=y[i]*result[float_gong];
                    }
                    fan_jinwei_c(y,leny,leny);
            ...
            while(jindu_x<=lenx)
            {
                if(flooat_gong>=result_long)
                {
                    return;
                }
                result[float_gong]=part_chu_t(x,y,leny,temp_y);
                
                    for(int i=0;i<leny;i++)
                    {
                        x[i+1]-=y[i]*result[float_gong];
                    }
                    fan_jinwei_c(x,leny+1,leny+1);
...
/*inline void part_chu_loop(char *temp,char *y,int leny,char *result)
{
    if(!(result[float_gong]=part_chu_t(temp_x,y,leny,temp))&&!result[float_gong-1]&&!result[float_gong-2])
    {
        ;
    }
    for(int i=0;i<leny;i++)
    {
        temp_x[i+1]-=y[i]*result[float_gong];
    }
    fan_jinwei_c(temp_x,leny+1,leny+1);
    float_gong++;
    fake_strncpy(temp_x,temp_x+1,leny);
}*/
